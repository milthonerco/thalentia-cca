...
export async function inscribirEstudiante(
  ...
  */
  ...
  if (!estudiante) {
    throw new Error("Perfil estudiante no encontrado");
  }
  ...
  */
  ...
  DEL ESTUDIANTE
  */
  const academia = academias.find(
    a => a.cursos_permitidos?.includes(estudiante.curso)
  );

  if (!academia) {
    throw new Error("Esta academia no está disponible para tu curso");
  }
  ...
  */
  ...
  ==========================
  */
  if (academia.inscritos_actuales >= academia.cupo_maximo) {
  ...
  */
  ...
  if (count && count >= 2) {
    throw new Error("Máximo dos academias");
  }

  /*
  ==========================
  YA INSCRITO (SOLO ACTIVAS)
  ==========================
  */
  ...
  if (existente) {
    throw new Error("Ya estás inscrito");
  }
  ...
  */
  ...
  if (insertError) {
    ...
    throw new Error("No fue posible registrar inscripción");
  }
  ...
  */
  const { error: updateError } = await supabaseAdmin
    .from("academias")
    .update({
      inscritos_actuales: academia.inscritos_actuales + 1
    })
    .eq("id", academia.id);

  if (updateError) {
    throw new Error("No fue posible actualizar cupos");
  }
